...
# --- Visualization with Background Coloring ---
with col_main:
    ...
    if len(cb) >= 1:
        ...
        # Flatten grid and find nearest centroid for every pixel
        grid_points = np.c_[xx.ravel(), yy.ravel()]
        dist_sq = np.sum((grid_points[:, np.newaxis, :] - cb[np.newaxis, :, :]) ** 2, axis=2)
        Z = np.argmin(dist_sq, axis=1)
        Z = Z.reshape(xx.shape)
        
        # Plot the colored regions
        # cmap='Pastel1' or 'tab20c' works well for light backgrounds
